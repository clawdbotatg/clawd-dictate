...
- **Mic between dictations:** held OPEN for 24 h after the last one (orange dot). Tested 09-15: iOS refuses to reopen the mic from the background, so releasing it means a hop into the app on every keyboard use. Austin chose hold. Audio streams to Deepgram only while listening — the tap drops buffers otherwise.
